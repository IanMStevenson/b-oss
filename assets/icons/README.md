...
└── entitlements.mac.plist # macOS hardened-runtime entitlements                  [done]
```

`entitlements.mac.plist` is committed directly under `assets/icons/`. `icon.icns` is generated by
`scripts/make-icns.sh` (macOS-only — run from `release-mac.yml` or locally before
`electron-builder --mac`) from the `icon.iconset/` sources below, and is gitignored like the rest
of this directory's copies under `packages/b-ark/resources/`.

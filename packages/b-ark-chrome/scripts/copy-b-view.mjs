...
// Mirrors the built standalone viewer (packages/b-view-backup/dist-app/) into this
...
// backup folder via deployViewer(). The copy is gitignored — b-view-backup is the single
// source. Run by this package's build/dev/dist scripts, after b-view-backup itself has
// been built.
...
const src = resolve(packageRoot, '../b-view-backup/dist-app');
const dest = resolve(packageRoot, 'public/b-view-dist');

if (!existsSync(src)) {
  console.error(
    `[copy-b-view] b-view dist not found at ${src} — run "npm run build --workspace=packages/b-view-backup" first`,
  );
  process.exit(1);
}
...
console.log(`[copy-b-view] b-view-backup/dist-app → public/b-view-dist (${files.length} files)`);

...
import type { AccountConfig, LogEntry, LogCsvFilters } from './backend.js';
...
interface LogPanelProps {
  /** All known accounts (for the journal filter + username column). */
  accounts: AccountConfig[];
  /** Live per-account log buffers streamed in from the host. */
  liveLogBuffer: Record<string, LogEntry[]>;
  /** Close the panel. */
  onClose: () => void;
  /** Load historic (persisted) logs on mount. */
...
export function LogPanel({
  ...
  const allLogs = (() => {
    ...
    for (const id of Object.keys(liveLogBuffer)) {
      for (const e of liveLogBuffer[id] ?? []) liveAcrossAccounts.push(e);
    }
  ...
  const liveTotal = Object.values(liveLogBuffer).reduce((a, b) => a + b.length, 0);
  useEffect(() => {
    if (!paused) {
